# v2: 4/24/2017 Andrew Chang: allow user to select customed DNA condons (mainly for PetiDream platform)
...
def def_translate(rootdir, file_name_list, limit, amber_stop, custom_dict_input):
    """
    ...
    if custom_dict_input == None:
        ...
        base2aa = custom_dict_input # use user defined codons
...
custom_dict = raw_input("Use customed codons? (T/F or simply skip) ")
if (custom_dict == 'T'):
    root1=Tkinter.Tk() # if you are using Canopy, please go to 'Preferences > Python > PyLab Backend: Inline(SVG)'
    root1.withdraw()
    root1.update()
    codon_file_path = tkFileDialog.askopenfilename(title='Please select your codons (.csv)')
    root1.destroy()
    codon_file = pd.read_csv(codon_file_path, header=None, index_col = 0)
    custom_dict_input = codon_file.to_dict(orient='dict')[1] # converting the customed codon file to a dict.
else:
    custom_dict_input = None
...
def_translate(rootdir,file_name_list, min_length, amber_stop, custom_dict_input) 
