# v4 AC 3/8/2017: add %ID column to the output csv
# v3 AC 12/5/2016: allow user to 1. whether to generate fasta file for each cluster. 2. input flanking seqs, and cluster seq based on just the variable position
...
## User Inputs ##
gfasta = raw_input("Generate fasta files for each cluster? (Y or N): ")
threshold = raw_input("Input the sequence alignment threshold (0.75): ")
PerOrRank = raw_input("Generate Cluster Heatmap based on either (1)Percentage or (2)RankScore: ")
flank_seqs = raw_input('Input the flanking sequences and separate them with comma, e.g. LEGC, CKQD, GCVC,...(or simply skip this step) ')
...
## extracting the variable positions by flanking sequences ##
if flank_seqs == '':
    print "Cluster ALL the sequences without extracting the variable regions..."
    my_seqs=[]
    for i, peptide in enumerate(test["Peptide"]):
        my_seqs.append(SeqRecord(Seq(str(peptide),IUPAC.protein), id = str(i)))
        #print(i)
else:
    flank_seqs = flank_seqs.split(',')
    flank_seqs = [x.strip() for x in flank_seqs] # all the flanking seqs are in the list flank_seqs

    flank_new = ''
    for flank in flank_seqs:
        flank_new = flank_new + flank +'(.*)'
    flank_new = flank_new[:-4] # remove the last (.*)
        
    temp_seq = test['Peptide'].str.extract(flank_new) 
    flanked_n = len(flank_seqs)-1 # number of flanked sequences
    tick = 0
    test['var_seq'] = ''
    
    if flanked_n == 1: # if there is only one flanked seq
        test['var_seq'] = temp_seq
    else: # if there are more than one flanked seqs           
        while tick <= flanked_n:
            test['var_seq'] = test.var_seq.str.cat(temp_seq[tick], sep='')
            tick = tick + 2
    
    ## start to generate fasta file for the sequences that will be clustered ##
    my_seqs=[]
    for i, peptide in enumerate(test["var_seq"]):
        my_seqs.append(SeqRecord(Seq(str(peptide),IUPAC.protein), id = str(i)))
        #print(i)
...
data = pd.read_table(outfile3, 
    sep='\t',
    names=['Record Type','Cluster_ID','Seq_Count','H_%id','H_strand','na','na','note','Label','Cluster rep'],
    na_values=['*'])

data['Label'] = data['Label'].str.split(' ').str.get(0).astype(int)
data['Cluster rep'] = data['Cluster rep'].astype(str).str.split(' ').str.get(0) # this is still string not integer. need to convert it during accessing.
cluster_num = data['Cluster_ID'].unique()
test['Cluster'] = 'NA' # initialize a new column for Cluster
test['Cluster_Size'] = 'NA' # initialize a new column for Cluster Size
test['%ID'] = 'NA'

# pull up the # of seq for each cluster from the .uc file and save it to a new dataframe, cluster_stat
cluster_stat = data.loc[data['Record Type'] == 'C',['Cluster_ID','Seq_Count']] 
cluster_stat['%ID'] = 'NA'
...
for value in cluster_num:
    ...
    test.loc[idx,['Cluster_Size']] = int(cluster_stat.loc[cluster_stat.Cluster_ID == value,'Seq_Count'])
    ...
    test.loc[idx,'%ID'] = data.loc[(data['Record Type'] == 'H')&(data['Cluster_ID']==value),'H_%id'].mean() 
