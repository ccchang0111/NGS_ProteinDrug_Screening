# v2-5 AC 12/5/2016: adapted for modified 2_Clean_COUNTcsv.py. extract minimum percentage from imported file and used for S/N calculation for those 'control' seq not existing in the test sample.
...
test = test.iloc[:,:-1] # remove the column of min%
...
for control_file_path in control_file_paths:
    ...
    controls_min.append(control['min%'][0]) # this will be used in later S/N calc for those control seqs that do not exist in test file
    control = control.iloc[:,:-1] # remove the column of min%
...
for i, control_percentage_name in enumerate(control_percentage_names):
    idx3 = overlap[control_percentage_name].isnull() # find all the NaN cells in the controls
    overlap.loc[idx3,control_percentage_name] = controls_min[i] # fill those NaN cells with the min value that is extracted from previous set
